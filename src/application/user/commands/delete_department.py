...
class DeleteDepartment(Command[None]):
    user_id: UUID


class DeleteDepartmentHandler(CommandHandler[DeleteDepartment, None]):
    def __init__(
        self,
        user_repo: UserRepo,
        uow: UnitOfWork,
    ...
    async def __call__(self, command: DeleteDepartment) -> None:
        ...
        logger.info("User department deleted", extra={"user": user})

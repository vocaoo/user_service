import logging

import orjson
from aio_pika import ExchangeType, DeliveryMode, Message as RMQMessage
from aio_pika.abc import AbstractChannel, AbstractExchange

from .interface import MessageBroker
from .message import Message

logger = logging.getLogger(__name__)


class MessageBrokerImpl(MessageBroker):
    def __init__(self, channel: AbstractChannel) -> None:
        self._channel = channel

    async def publish_message(
        self,
        message: Message,
        routing_key: str,
        exchange_name: str,
    ) -> None:
        rq_message = self.build_message(message)
        await self._publish_message(rq_message, routing_key, exchange_name)

    async def declare_exchange(self, exchange_name: str) -> None:
        await self._channel.declare_exchange(exchange_name, ExchangeType.TOPIC)

    @staticmethod
    def build_message(message: Message) -> RMQMessage:
        return RMQMessage(
            body=orjson.dumps(dict(message_type=message.message_type, data=message.data)),
            message_id=str(message.id),
            content_type="application/json",
            delivery_mode=DeliveryMode.PERSISTENT,
            headers={},
        )

    async def _publish_message(
        self,
        rq_message: RMQMessage,
        routing_key: str,
        exchange_name: str,
    ) -> None:
        exchange = await self._get_exchange(exchange_name)
        await exchange.publish(rq_message, routing_key=routing_key)
        logger.debug("Message sent", extra={"rq_message": rq_message})

    async def _get_exchange(self, exchange_name: str) -> AbstractExchange:
        return await self._channel.get_exchange(exchange_name, ensure=False)

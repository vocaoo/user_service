...
from src.application.user.interfaces.persistence import GetUserFilters, UserReader, UserRepo
...
class UserReaderImpl(SQLAlchemyRepo, UserReader):
    ...
    @exception_mapper
    async def get_users(self, filters: GetUserFilters, pagination: Pagination) -> dto.Users:
        query = select(User)
    ...
    async def _get_users_count(self, filters: GetUserFilters) -> int:
...
class UserRepoImpl(SQLAlchemyRepo, UserRepo):
    ...
    def _parse_error(self, err: DBAPIError, user: entities.User) -> NoReturn:
        match err.__cause__.__cause__.constraint_name:  # type: ignore
            case "pk_users":
                raise UserIdAlreadyExists(user.user_id.to_raw()) from err
